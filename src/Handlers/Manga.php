...
class Manga extends App
{
    public function show($param)
    {
        ...
        if ($slug == 'catalog')
        {
            ...
            $sth = $this->db->prepare("SELECT t.*, tt.*, g.name AS genre FROM titles t
                                    JOIN titles_add_text tt
                                    ON t.id = tt.title_id
            ...
            $this->renderer->display('Manga', $data);
        }
    ...
    public function catalogShow ()
    {
        $sth = $this->db->query("SELECT t.id, t.name, t.name_alt, t.img, t.type AS 'type', t.new, t.slug,
                                        tt.type AS type_type, tt.chapters, 
        ...
        $this->renderer->display('MangaCatalog', $data);
    }
}
